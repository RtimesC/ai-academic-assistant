  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
      <div style={{ background: '#fff', borderRadius: 12, padding: 24, width: 480, maxWidth: '95vw', boxShadow: '0 8px 32px rgba(0,0,0,0.15)' }}>
        <h2 style={{ fontSize: 18, fontWeight: 700, marginBottom: 20 }}>录入健康数据</h2>
        {error && <div style={{ background: '#fadbd8', color: '#c0392b', padding: 8, borderRadius: 5, marginBottom: 12 }}>{error}</div>}
        <form onSubmit={handleSubmit}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {[['systolic_bp','收缩压 (mmHg)'],['diastolic_bp','舒张压 (mmHg)'],['heart_rate','心率 (bpm)'],['blood_glucose','血糖 (mg/dL)'],['weight','体重 (kg)'],['temperature','体温 (°C)'],['mmse_score','MMSE评分 (0-30)']].map(([name, label]) => (
              <div key={name}>
